...
class HiddenMarkovModel:
    ...
    def __init__(self, A, O, get_syls = False):
        '''
    ...
    def generate_sonnet_emission(self, syl):
        emission = []
        states = []
        syl_count = 0
        state = np.random.choice(range(self.L))
        add = np.random.choice(range(self.D), p = self.O[state])
        emission.append(add)
        states.append(state)
        syl_count += int(self.syls[add])
        while syl_count < syl:
            add = np.random.choice(range(self.D), p = self.O[state])
            add_syls = int(self.syls[add])
...
def unsupervised_HMM(X, n_states, N_iters, get_syls = False):
    '''
    ...
    HMM = HiddenMarkovModel(A, O, get_syls = get_syls)
