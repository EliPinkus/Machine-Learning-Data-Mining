...
def generate_traindata(word_list, word_to_index, window_size=4):
    ...
    for i in range(len(word_list)):
    	...
    	for j in range(low, high +1):
    		if j >= 0 and j < len(word_list) and i != j:
    			trainX.append(x)
...
if __name__ == "__main__":
    ...
    fit = model.fit(trainX, trainY, batch_size = 128, epochs = 10, verbose = 1)

    weights = model.layers[0].get_weights()[0]
    weights2 = model.layers[1].get_weights()[0]

    print("Weights shape layer 1: " + str(weights.shape))
    print("Weights shape layer 2: " + str(weights2.shape))
